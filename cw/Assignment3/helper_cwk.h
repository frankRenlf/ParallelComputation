//
// Modified from the version supplied to students to also output the matrix and both vectors
// to file, the same time they are displayed.
//
...
void displayMatrixAndVector(float *M, float *x, int N)
{
	...
	if (N > 8)
		printf(" (only showing top-left 8x8 elements)");
	printf(":\n");
	...
	if (N > 8)
		printf(" (only showing first 8 elements)");
